...
export function PipelineRail() {
  const { stage, goto, reachable, phase } = usePE();
  const activeIndex = STAGES.findIndex((s) => s.id === stage);

  return (
    <nav className="flex h-full w-[248px] shrink-0 flex-col panel !rounded-none !rounded-r-xl border-l-0 py-4">
      ...
      <div className="relative flex-1 overflow-y-auto px-3">
        <div className="absolute left-[27px] top-1 bottom-1 w-px bg-hairline" />
        <motion.div
          className="absolute left-[27px] top-1 w-px"
          style={{
            background: "linear-gradient(var(--flux), transparent)",
          }}
